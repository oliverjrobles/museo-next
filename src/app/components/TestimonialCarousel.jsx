...
export default function TestimonialsCarousel() {
  const [items, setItems] = useState([]); // ⬅ ingen <...> her
    ...
    <section className="py-16 md:py-20 text-white bg-cover bg-center relative" style={{ backgroundImage: "url('/assets/bg/footerbg.jpg')" }}>
      ...
      <div className="relative z-10 max-w-md md:max-w-4xl mx-auto px-4 sm:px-6 text-center">
        ...
        <div className="overflow-hidden">
          <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${safeCurrent * 100}%)` }}>
            {items.map((item, index) => (
              <div key={item.id} className="min-w-full flex flex-col items-center">
                <div className={`flex flex-col items-center transition-opacity duration-500 ease-out ${index === safeCurrent ? "opacity-100" : "opacity-0"}`}>
                  {item.imageUrl && (
                    <div className="mb-6 md:mb-8 flex justify-center">
                      <img src={item.imageUrl} alt={item.name} className="w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 object-cover rounded-sm shadow-lg" />
                    </div>
                  )}

                  <h3 className="tracking-[0.4em] text-[11px] sm:text-xs md:text-sm uppercase mb-4 md:mb-6">{item.name}</h3>

                  <p className="text-xs sm:text-sm md:text-base leading-relaxed opacity-90 max-w-md md:max-w-3xl mx-auto">{item.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 md:mt-8 flex justify-center gap-2">
          {items.map((_, index) => (
            <button key={index} onClick={() => setCurrent(index)} aria-label={`Gå til testimonial ${index + 1}`} className={`w-3 h-3 md:w-4 md:h-4 rounded-[3px] border-2 border-[#ff3e7f] transition-colors ${index === safeCurrent ? "bg-[#ff3e7f]" : "bg-white hover:bg-[#ff3e7f]"}`} />
          ))}
        </div>
      </div>
    </section>
